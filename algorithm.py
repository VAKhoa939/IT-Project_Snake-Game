...
class Algorithm:
    def __init__(self, snake_parts_id : list[tuple[int, int]], noises_id : list[tuple[int, int]]):
    ...
    def input_current_state(self, snake_parts_id : list[tuple[int, int]], other_snake_parts_id : list[tuple[int, int]], food_id : tuple[int, int]):
        self.snake_parts_id = snake_parts_id
    ...
    def find_neighbors(self, node : Node):
        ...
        invalid_id = self.snake_parts_id + self.other_snake_parts_id + self.noises_id + frontier_id + visited_id
    ...
    def get_path(self, node : Node):
        while node.prev_id != (-1, -1):
            position = (node.prev_id[0] * CELL_SIZE + BOARD_OFFSET + PART_SIZE, node.prev_id[1] * CELL_SIZE + BOARD_OFFSET + PART_SIZE)
            direction = self.direction_dict[(node.id[0] - node.prev_id[0], node.id[1] - node.prev_id[1])]
